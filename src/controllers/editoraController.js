...
const inspecionaEditora = async (req, res) => {
	try {
		...
		res.status(200).render("updateEditora", { editora }); // trocar pela página de atualizar e deletar editora
	} catch (error) {
		res.status(500).send(error.message);
	}
};
...
const atualizarEditora = async (req, res) => {
	try {
		const editora = await EditoraModel.findByIdAndUpdate(
			req.params.id,
			req.body
		);
...
const buscaPLocal = async (req, res) => {
	try {
		const editoras = await EditoraModel.find({
			localDeOrigem: { $regex: req.body.buscaLocal, $options: "i" },
		});
